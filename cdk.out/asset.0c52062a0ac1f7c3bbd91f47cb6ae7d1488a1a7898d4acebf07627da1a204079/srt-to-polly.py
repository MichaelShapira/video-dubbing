...
def run_polly_job(text, duration,unique_id):
    ...
    response = polly_client.start_speech_synthesis_task(
        ...
        LanguageCode='ru-RU',
        OutputFormat='mp3',
        OutputS3BucketName='dubbing-output',
        OutputS3KeyPrefix=f'{unique_id}/polly_output/',
        Text=polly_text,
        TextType='ssml',
        VoiceId='Tatyana',
        SnsTopicArn='arn:aws:sns:us-east-1:346399954218:DubbingPollyJobs'
    )
...
def lambda_handler(event, context):
    ...
    media_url = translated_srt['media']
    chunks_counter = len(translated_srt['subs'])
    media_output_bucket = 'dubbing-output'
